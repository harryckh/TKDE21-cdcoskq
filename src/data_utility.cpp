...
coskq_config_t* read_config_coskq()
{
    ...
    fscanf(c_fp, "%i", &cfg->w_opt);

    //algorithm option.
    fscanf(c_fp, "%i", &cfg->alg_opt);

    //method option.
    fscanf(c_fp, "%i", &cfg->m_opt);
 ...
 * for IR-tree
 */
void add_keyword_entry_with_weight(k_node_t*& k_node_v, KEY_TYPE key, W_TYPE weight)
{
    k_node_v->next = (k_node_t*)malloc(sizeof(k_node_t));
    memset(k_node_v->next, 0, sizeof(k_node_t));
    k_node_v->next->key = key;

    k_node_v->next->weight = weight;
...
void print_coskq_stat(coskq_config_t* cfg, int cnt)
{
	...
	//average ratio ab .
	fprintf(s_fp, "%lf\n%lf\n", stat_v.ratioa_aver,stat_v.ratiob2_aver);
 ...
 */
data_t* read_data_coskq(coskq_config_t* cfg, std::unordered_map<KEY_TYPE, KEY_TYPE>* keyfreq_hashmap, W_TYPE& max_weight)
{
    int i, j;
    KEY_TYPE key;
    char des;
    ...
    for (i = 0; i < data_v->obj_n; i++) {
        ...
        if (cfg->tree_tag == 1)
            // object weight
			fscanf(i_fp, "%c%d", &des, &(data_v->obj_v[i].weight));
		
        else {
            //generate by normal distribution with mean

            double number = distribution(generator);
            while (!(number >= 1.0) && (number <= 100.0)) {
                number = distribution(generator);
            }
            data_v->obj_v[i].weight = int(number);
            //		printf("%d\t", int(number));
        }
		
		if(data_v->obj_v[i].weight> max_weight)
			max_weight = data_v->obj_v[i].weight;
        //-------
...
void print_data(data_t* data_v, syn_config_t* s_cfg)
{
    ...
    if ((loc_fp = fopen(s_cfg->new_loc_file, "w")) == NULL || (doc_fp = fopen(s_cfg->new_doc_file, "w")) == NULL) {
