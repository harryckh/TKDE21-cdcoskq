...
void release_psi(psi_t* psi_v)
{
    ...
    /*s*/
    stat_v.memory_v = stat_v.memory_v - sizeof(psi_t) - (psi_v->key_n + 1) * sizeof(k_node_t);
    /*s*/
...
//--
/*
 *	Allocate the memory for a ellipse_t structure.
 */
ellipse_t* alloc_ellipse(int dim)
{
    ellipse_t* ellipse_v;

    ellipse_v = (ellipse_t*)malloc(sizeof(ellipse_t));
    memset(ellipse_v, 0, sizeof(ellipse_t));

    ellipse_v->center1 = alloc_loc(dim);
    ellipse_v->center2 = alloc_loc(dim);

    /*s*/
    stat_v.memory_v += sizeof(ellipse_t);
    if (stat_v.memory_v > stat_v.memory_max)
        stat_v.memory_max = stat_v.memory_v;
    /*s*/
...
/*
 *	Initialize a ellipse_t structure.
 */
void set_ellipse(ellipse_t* ellipse_v, loc_t* loc_v1, loc_t* loc_v2, B_KEY_TYPE diam)
{
    int i;
    ...
    for (i = 0; i < loc_v1->dim; i++)
        ellipse_v->center1->coord[i] = loc_v1->coord[i];
    for (i = 0; i < loc_v2->dim; i++)
        ellipse_v->center2->coord[i] = loc_v2->coord[i];
}

/*
 *	Construct a ellipse_t structure with its center of @loc_v and
 *	its radius of @radius.
 */
ellipse_t* const_ellipse(loc_t* loc_v1, loc_t* loc_v2, B_KEY_TYPE diam)
{
    int i;
    ellipse_t* ellipse_v;

    ellipse_v = alloc_ellipse(IRTree_v.dim);
    for (i = 0; i < IRTree_v.dim; i++)
        ellipse_v->center1->coord[i] = loc_v1->coord[i];
    ellipse_v->center1->dim = IRTree_v.dim;

    for (i = 0; i < IRTree_v.dim; i++)
        ellipse_v->center2->coord[i] = loc_v2->coord[i];
    ellipse_v->center2->dim = IRTree_v.dim;
...
/*
 *	Release a ellipse_t structure.
 */
void release_ellipse(ellipse_t* ellipse_v)
{
    if (!ellipse_v)
        return;

    release_loc(ellipse_v->center1);
    release_loc(ellipse_v->center2);
    free(ellipse_v);
...
void release_obj_set(obj_set_t* obj_set_v)
{
    ...
    /*s*/
    stat_v.memory_v -= sizeof(obj_set_t) + (obj_set_v->obj_n + 1) * sizeof(obj_node_t);
    /*s*/
 ...
 ** return all children's positions that contain query keyword by 32-bits (e.g., 1001)
 */
...
void add_obj_set_entry(obj_t* obj_v, obj_set_t* obj_set_v)
{
    ...
    //--
    obj_node_v->IF_S = NULL;
    obj_node_v->range = NULL;
    obj_node_v->maxInvalidRange = 0;
    //--
 ...
 *	Compute the farthest distance between a set of objects @obj_set_v and @q.
 */
B_KEY_TYPE comp_farthest(obj_set_t* obj_set_v, loc_t* loc_q)
{
    B_KEY_TYPE far_dist, dist;
    loc_t* loc_v;
    obj_node_t* obj_node_v;
    ...
    while (obj_node_v != NULL) {
        ...
        dist = calc_dist_loc(loc_v, loc_q);
        if (dist > far_dist)
            far_dist = dist;
...
B_KEY_TYPE comp_cost(int cost_tag, obj_set_t* obj_set_v, query_t* q)
{
    ...
    case Cost_MaxMax2: //Dia
        return fmax(comp_farthest(obj_set_v, q), comp_diameter(obj_set_v));
			/*case 13: //MaxDotSize
        return comp_farthest(obj_set_v, q) * comp_weight_sum(obj_set_v);
    case 4:
        return (comp_diameter(obj_set_v) / MAX_DIA) + (comp_weight_max(obj_set_v) / MAX_WEIGHT);
...
/*
B_KEY_TYPE comp_cost(int cost_tag, obj_set_t* obj_set_v, loc_t* loc_v)
{
    //    printf("cost_tag:%d\n",cost_tag);
    switch (cost_tag) {
    case Cost_MaxMax: //MaxSum
        return comp_farthest(obj_set_v, loc_v) + comp_diameter(obj_set_v);
    case Cost_MaxMax2: //Dia
        return fmax(comp_farthest(obj_set_v, loc_v), comp_diameter(obj_set_v));
    default: //error
        return 0;
    }
}*/
...
obj_set_t* ConstructFeasibleSet_Exact(obj_t* o, query_t* q, bst_t* obj_pair)
{
    ...
    while (cur_pair != NULL) {
        ...
        ///alg2 line 1
        if (calc_dist_obj(triplet_v.o_1, triplet_v.o_2) < fmax(calc_dist_obj(triplet_v.o, triplet_v.o_1),
...
void collect_key_freq(data_t* data_v, int* freq_v, int size)
{
    ...
    for (i = 0; i < data_v->obj_n; i++) { //printf( "%d:\t",i);
...
/*
 *	The implementation of query generation in TODS15 SGKQ
 *
 */
query_t** gen_query_set_Cao(int query_n, int key_n, int obj_n, data_t* data_v)
{
    int i, j, k, cnt, k_max, k_num, rand_tmp, rand_tmp2;
    float p_low, p_high;
    int *rand_key, *rand_obj;
    loc_t* loc_v;
    psi_t* psi_v;
    query_t** q_set;
    key_freq_pair_t* key_freq_pair_v;
    ...
    printf("Generating queries by Cao Method...\n");
    ...
    for (i = 0; i < query_n; i++) {
        ...
        rand_obj = (int*)malloc(key_n * sizeof(int));
        memset(rand_obj, 0, sizeof(int));
        rand_key = (int*)malloc(key_n * sizeof(int));
        memset(rand_key, 0, sizeof(int));
        ...
        for (k = 0; k < key_n; k++) {
            ...
            do {
                ...
                do {
                    ...
                    rand_tmp = rand_i(0, obj_n - 1);

                    //count the number of keys in this object
                    k_node_v = data_v->obj_v[rand_tmp].k_head->next;
                    cnt_key = 0;
                    while (k_node_v != NULL) {
                        cnt_key++;
                        k_node_v = k_node_v->next;
                    }
                } while (is_old(rand_obj, k, rand_tmp) || cnt_key == 0);

                //  printf("cnt_key:%d\n", cnt_key);

                //picking a key from the object
                rand_tmp2 = rand_i(0, cnt_key - 1);
                k_node_v = data_v->obj_v[rand_tmp].k_head->next;
                cnt_key = 0;
                while (k_node_v != NULL) {
                    if (rand_tmp2 == cnt_key)
                        break;
                    cnt_key++;
                    k_node_v = k_node_v->next;
                }
                key = k_node_v->key;
            } while (is_old(rand_key, k, key));

            rand_obj[k] = rand_tmp;
            rand_key[k] = key;

            add_psi_entry(psi_v, key);
        }
        // printf( "\n");
        ...
        for (j = 0; j < data_v->dim; j++) {
            double sum = 0.0;
            for (k = 0; k < key_n; k++) {
                sum += data_v->obj_v[rand_obj[k]].MBR[j].min;
            }
            loc_v->coord[j] = sum / key_n;
        }
        q_set[i]->loc_v = loc_v;

        //		print_query(q_set[i], stdout);
