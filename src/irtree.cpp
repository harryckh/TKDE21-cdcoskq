...
bool key_oriented_check( node_t* node_v)
{
	...
	for( i=0; i<node_v->num; i++)
	{
		if( node_v->level == 0)
		{
			...
			k_head = collect_keywords_list( ( ( obj_t*)( node_v->child[ i]))->k_head, ( ( obj_t*)( node_v->child[ i]))->weight);
		}
		else	//node_v->level > 0
		{
	...
	while( get_next_in_order( x, tag))
	{
		...
		if( p_tag == 1)
		{
			...
			for( i=0; i<M; i++)
			{
				if( get_k_bit( x->p_list, i)){
					fprintf( fp, "%i", i);
					fprintf( fp, "(%i) ", x->p_list_weight[i]); //new
				}
			}
			fprintf( fp, "\n");
		}
 ...
 */
k_node_t* collect_keywords_list( k_node_t* k_head, W_TYPE weight)
{
...
//		add_keyword_entry( k_node_v1, k_node_v2->key);
		add_keyword_entry_with_weight( k_node_v1, k_node_v2->key, weight);
...
k_node_t* collect_keywords_bst( bst_t* bst_v)
{
	...
	while( get_next_in_order( x, tag))
	{
		add_keyword_entry_with_weight( k_node_v1, x->key, x->min_weight);
 ...
 * *	return the updated max weight.
 */
int add_IF_entry( bst_t* bst_v, KEY_TYPE key, int loc, W_TYPE weight)
{
	bst_node_t* bst_node_v;
	...
	if( bst_node_v != NULL)
	{
		...
		//---
		//new
		bst_node_v->p_list_weight[loc] = weight;
		if(weight < bst_node_v->min_weight){
			bst_node_v->min_weight = weight;
			return weight;
		}
		//---
		return -1;
	...
	else	//bst_node_v = NULL
	{
		...
		///construct a new array to store the max weight of each child
		bst_node_v->p_list_weight = ( W_TYPE*)malloc( sizeof( W_TYPE)*M);
		memset( bst_node_v->p_list_weight, 0, sizeof( W_TYPE)*M);
		
		/*s*/
		stat_v.tree_memory_v += sizeof( bst_node_t) + sizeof(W_TYPE)*M;
		if( stat_v.tree_memory_v > stat_v.tree_memory_max)
			stat_v.tree_memory_max = stat_v.tree_memory_v;
		/*s*/
...
//		new_keys = collect_keywords_list( ( ( obj_t*)obj)->k_head);
		new_keys = collect_keywords_list( ( ( obj_t*)obj)->k_head,( ( obj_t*)obj)->weight);
	...
	while( true)
	{
		...
		while( k_node_v1 != NULL)
		{
			...
			res = add_IF_entry( c_node->bst_v, k_node_v1->key, loc, k_node_v1->weight);
//			if( res == 0)
			if( res == -1)
			{
				/// k_node_v1->key->weight is not larger than the current one
				/// no need to update ancestor
			...
			else	//res >0 and represent the (new) max_weight
			{
				//---
				k_node_v1->weight = res;
				//---
...
void const_IF( node_t* node_v)
{
	...
	{
		if( node_v->level == 0)
			k_node_v = collect_keywords_list( ( ( obj_t*)( node_v->child[ loc]))->k_head,( ( obj_t*)( node_v->child[ loc]))->weight );
		else	//
		...
		while( k_node_v1 != NULL)
		{
			add_IF_entry( node_v->bst_v, k_node_v1->key, loc, k_node_v1->weight);
